...
for j in range(len(figs)):
    ...
    if (j==0):
        fig.add_trace(go.Scatter(x=Gs, y=df_Eigen['Ecm1'], mode='lines', name='Ecm',
                                 line=dict(color='rgb(255,175,175)', width=2, dash='dot')))
#        fig.add_trace(go.Scatter(x=Gs, y=df_Eigen['Ecm2'], mode='lines', name='Ecm2'))
        fig.add_trace(go.Scatter(x=Gs, y=[E+RabiC/2 for E in df_Eigen['Ecm2']], mode='lines', name='Ecm+C',
                                 line=dict(color='rgb(175,175,255)', width=2, dash='dot')))
        fig.add_trace(go.Scatter(x=Gs, y=[E-RabiC/2 for E in df_Eigen['Ecm2']], mode='lines', name='Ecm-C',
                                 line=dict(color='rgb(175,255,175)', width=2, dash='dot')))
...
VecFigR.update_layout(title= 'Vector Coordinates',
                  ...
                  yaxis_title='Coefficient',
                  showlegend=True)
VecFigR.write_html('plots/' + '2x2_VectorCoordinatesReal.html', auto_open=False)
...
VecFigC.update_layout(title= 'Vector Coordinates',
                  ...
                  yaxis_title='Coefficient',
                  showlegend=True)
VecFigC.write_html('plots/' + '2x2_VectorCoordinatesComp.html', auto_open=False)

phaseTransPlot = go.Figure()
phaseTransPlot.add_trace(go.Scatter(x=Gs, y=orderParameter1,
